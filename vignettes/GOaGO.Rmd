...
Also, gene pairs containing the same gene twice are excluded by GO-a-GO as non-informative.
...
When running the main function `GOaGO`, we should specify that the gene identifiers are from the Entrez database.
...
```{r dotplot, fig.cap = "Dotplot of enriched Gene Ontology terms"}
DotPlot(goago)
```
...
```{r ridgeplot, fig.cap = "Ridgeplot of enriched Gene Ontology terms"}
RidgePlot(goago)
```

Given the shape of the above distributions, the low *p*-values are understandable.

## Comparison to existing Gene Ontology enrichment approaches
...
Note that for a fair comparison we will exclude self-loops, i.e. gene pairs (A, A), as these pairs are ignored by GO-a-GO.
...
```{r dotplot-ego, fig.cap = "Dotplot of enriched Gene Ontology terms, using unpaired approach by enrichGO"}
DotPlot(ego)
```
...
In these two objects we are have calculated statistics for `r format(nrow(as.data.frame(goago_relaxed)), big.mark = ",")` Gene Ontology terms that are shared by at least one gene pair, and separately for `r format(nrow(as.data.frame(ego_relaxed)), big.mark = ",")` terms that are associated to at least one gene.
...
For better visibility, log scale is used for both axes.
